    ...
    match endpoint.as_str() {
        ...
        "vertex_ai" => {
            ...
            let token = if crate::auth::has_service_account_key() {
                ...
                return Err("Vertex AI requires a service account. Place your key at ~/.gemini-workbench/vertex-key.json".to_string());
            };
        ...
        "openrouter" => {
            request = request
                ...
                .header("HTTP-Referer", "https://gemini-workbench.app")
                .header("X-Title", "Gemini Workbench");
        }
        "xai" | "kilocode" => {
...
pub async fn generate_image(
    ...
    let client = Client::new();
    let model = model_id.unwrap_or_else(|| "gemini-3-pro-image".to_string());
    let url = format!(
    ...
    if let Some(steps) = body.get("steps").and_then(|s| s.as_array()) {
        for step in steps.iter().rev() {
            ...
            if step_type == "model_output" {
                ...
                if let Some(text) = step.get("text").and_then(|t| t.as_str()) {
                    if !text.is_empty() {
...
pub async fn deep_research(
    ...
    for attempt in 0..max_attempts {
        ...
        match status {
            ...
            "running" | "pending" | "in_progress" | "requires_action" => {
                continue;
            }
            _ => {
...
pub async fn rag_query(api_key: String, store_names: Vec<String>, query: String, model: String) -> Result<Value, String> {
    let client = Client::new();
    let model_id = if model.is_empty() { "gemini-3.5-flash" } else { &model };
...
pub async fn veo_generate_video(
    ...
    let url = format!(
        ...
        AI_STUDIO_ENDPOINT, model_id, api_key
    );
